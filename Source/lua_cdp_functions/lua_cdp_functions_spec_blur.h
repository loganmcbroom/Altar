...
LUA_CDP_MULTI( lua_cdp_blur_chorus_1, 2, 0,{								// infile | mode | spread 1
	...
	lua_pushto( L, WAV_TYPE, 5 );		// blur | chorus | mode | inFile | $ | spread 1
	return cdp( L );
	})
LUA_CDP_MULTI( lua_cdp_blur_chorus_2, 3, 0,{								// infile | mode | spread 1 | spread 2
	...
	lua_pushto( L, WAV_TYPE, 5 );		// blur | chorus | mode | inFile | $ | spread 1 | spread2
	return cdp( L );
	})
...
void register_lua_cdp_functions_blur( lua_State * L )
	{
	lua_register( L, "blur_avrg",		lua_cdp_blur_avrg		);
	lua_register( L, "blur_blur",		lua_cdp_blur_blur		);
	lua_register( L, "blur_chorus",		lua_cdp_blur_chorus		);
	...
	lua_register( L, "blur_weave",		lua_cdp_blur_weave		);
	}